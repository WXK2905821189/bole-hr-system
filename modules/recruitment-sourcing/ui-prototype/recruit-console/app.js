...
const TITLES = {
  dashboard: ['运营总览', '候选人寻源 · 控制台'],
  jobs: ['岗位 / JD', 'JD → 关键词 → 定时关键词轮询'],
  schedule: ['采集调度', '定时批量 · 频率受控 · 失败熔断'],
  candidates: ['候选人库', '触达状态 · 匹配分排序 · 脱敏导出'],
  engage: ['触达沟通', '话术库 · 自动打招呼 · 智能跟进 · 批次 · 复盘'],
  audit: ['审计留痕', '采集 / 候选触达 / 导出 · 不可篡改'],
...
async function loadDashboard() {
  ...
  $('#view-dashboard').innerHTML = `
    <div class="kpis">
      <div class="kpi b-brand"><div class="lab">今日索取简历</div><div class="val">${ov.kpis.today}<small>份</small></div><div class="delta">昨日同期 <b>+12%</b> · 日上限 200</div></div>
      <div class="kpi b-ok"><div class="lab">入库成功率</div><div class="val">${ov.kpis.successRate}<small>%</small></div><div class="delta"><b>达标</b> · 阈值 ≥90%</div></div>
      <div class="kpi b-info"><div class="lab">解析核心字段准确率</div><div class="val">${ov.kpis.accuracy}<small>%</small></div><div class="delta"><b>↑ 1.2pp</b> · 近7天</div></div>
      <div class="kpi b-warn"><div class="lab">活跃候选人 / 待处理</div><div class="val">${ov.kpis.active}<small>人</small></div><div class="delta">本周新增 <b>38</b> · 待人工核对 <b class="warn">12</b></div></div>
    </div>
    <div class="split2">
      <div class="card">
        ...
        <div class="card-bd">
          ...
          <p class="textfield" style="margin:16px 0 0;color:var(--text-sub);font-size:12.5px;line-height:1.7">下一周期：<b style="color:var(--text)">「前端工程师」</b> 今日 17:00 · 间隔护栏 3–8s 随机化 · 目标 4 账号轮询。</p>
        </div>
      </div>
      <div class="card">
        ...
        <div class="card-bd" style="display:flex;gap:18px;align-items:center">
          ...
          <div class="legend">
            ...
            <div class="row"><span class="sw" style="background:var(--warn)"></span>需人工复核<b>${ov.quality.review}</b></div>
            <div class="row"><span class="sw" style="background:var(--danger)"></span>解析失败<b>${ov.quality.fail}</b></div>
          </div>
        </div>
      </div>
    </div>
    <div class="card">
      ...
      <div class="card-bd">
        <div class="touchband">${((d) => [
          ['pending', '待跟', d.pending], ['engaging', '跟进中', d.engaging],
          ['replied', '已回复', d.replied], ['sleeping', '沉睡', d.sleeping],
        ].map(([k, lab, v]) => `<div class="ts" data-goto="candidates" data-f="tch=${k}">
            <span class="tb ${TM[k][0]}"><i></i>${lab}</span><b>${v}</b>
            <span class="tpct">${d.total ? Math.round(v / d.total * 100) : 0}%</span></div>`).join('')
        )(touchCounts())}
        </div>
        <p class="textfield" style="margin:14px 0 0;color:var(--text-sub);font-size:12px;line-height:1.7">点击状态卡片可在候选人库按对应触达状态筛选；指标口径与明细见「触达分析」。</p>
      </div>
    </div>
    <div class="card">
      <div class="card-hd"><h2>今日候选高峰（按匹配分）</h2><span class="hint">Top 6 · 点击行查看详情</span></div>
      <div class="card-bd flat">
        <div class="table-wrap"><table><thead><tr><th>候选人</th><th>年限</th><th>核心技能</th><th>状态</th><th>匹配分</th></tr></thead>
    ...
    <div class="card">
      ...
      <div class="card-bd" style="border-top:1px solid var(--border)">
        <div style="display:flex;flex-wrap:wrap;gap:8px;margin-bottom:14px">
          <span class="chip">间隔随机化 3–8s</span><span class="chip">会话上限 80/次</span><span class="chip">冷却 30min</span><span class="chip">熔断 3 连败</span><span class="chip">日上限护栏 200</span><span class="chip">敏感字段默认脱敏</span>
        </div>
...
async function loadEngage() {
  ...
  const pendRows = eng.pendingRes.length ? eng.pendingRes.map((p) => `
    <div class="egrow"><div style="flex:1;min-width:0">
      ...
      <span class="badge b-warn">${p.action}</span></div>`).join('')
  ...
  const campRows = eng.campaigns.map((cg) => `
    <div class="egrow" style="flex-direction:column;align-items:stretch;gap:8px">
      <div style="display:flex;align-items:center;gap:8px">
        <b class="mono" style="font-size:13px">${cg.id}</b><span class="tag">${cg.mode}</span>
  ...
  host.innerHTML = `
    ...
    <div class="split2">
      <div class="card"><div class="card-hd"><h2>自动收取简历 · F-1</h2><span class="hint">收到简历附件自动解析入库</span></div>
        <div class="card-bd">${pendRows}</div></div>
      <div class="card"><div class="card-hd"><h2>交换联系方式 · F-2</h2><span class="hint">我方联系方式 · 统一回填话术</span></div>
    ...
    <div class="split2">
      <div class="card"><div class="card-hd"><h2>打招呼批次 · F-3</h2><span class="hint">职位顺序 · 额度管控 · 风控护栏</span>
          <span class="t-right"><button class="btn ghost sm" id="egCampaign" type="button">＋ 新建批次</button></span></div>
    ...
    <div class="card"><div class="card-hd"><h2>数据复盘 · F-4</h2><span class="hint">口径对齐 PRD 3.2 · 指标来自触达领域库</span>
  ...
  $('#egCampaign').addEventListener('click', async () => { const r = await api.createCampaign('全部职位 · 顺序'); toast(`${r.note}（${r.id}）`); });
...
function initGlobal() {
  ...
  $('#btnTopRun').addEventListener('click', () => { show('schedule'); toast('请在采集调度中运行'); });
...
async function load(page) {
  ({ dashboard: loadDashboard, jobs: loadJobs, schedule: loadSchedule, candidates: loadCandidates, engage: loadEngage, audit: loadAudit, settings: loadSettings })[page]?.();
}
initNav(); initModal(); initGlobal(); load('dashboard');